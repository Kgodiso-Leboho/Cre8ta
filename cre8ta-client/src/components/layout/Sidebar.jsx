  ...
  return (
    <aside className="sidebar">
      ...
      <div style={{ padding: "12px 0", flex: 1 }}>
        ...
        {links.map(link => (
          <div key={link.id} className={`sidebar-link ${activeSection === link.id ? "active" : ""}`}
            onClick={() => onSection(link.id)} style={{ animation: "slideIn .3s ease both" }}>
      ...
      <div style={{ padding: 16, borderTop: "1px solid var(--border)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
          <Avatar name={role === "creator" ? "Amara Osei" : "Nike Brand"} size={36} ring />
          <div>
            <div style={{ fontSize: 13, fontWeight: 600 }}>{role === "creator" ? "Amara Osei" : "Nike Brand"}</div>
            <Badge variant="gold">{role === "creator" ? "Creator" : "Brand"}</Badge>
          </div>
        </div>
        <button className="btn btn-ghost btn-sm" style={{ width: "100%", justifyContent: "center", gap: 6 }}
          onClick={() => onNavigate("landing")}>
          <Icon size={14} />Log out
        </button>
      </div>
    </aside>
  );
